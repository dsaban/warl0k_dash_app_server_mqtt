...
# Config
# ======================
st.set_page_config("WARL0K Secure MQTT Dashboard", layout="wide")
...
if "history" not in st.session_state:
    st.session_state.history = pd.DataFrame(columns=[
        "timestamp", "device", "session_id", "secret", "noisy_input", "regenerated",
...
# Sidebar Controls
# ======================
st.sidebar.title("WARL0K Session Simulator")
...
# Main Display
# ======================
st.title("📡 WARL0K Secure Session Insights")

if st.session_state.sessions:
    selected = st.selectbox("Select Session", options=[f"{s['id']} ({s['device']})" for s in st.session_state.sessions])
    sess = next(s for s in st.session_state.sessions if s["id"] in selected)

    st.subheader(f"Session ID: {sess['id']} — Device: {sess['device']}")
