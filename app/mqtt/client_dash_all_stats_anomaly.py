...
st.title("🔐 WARL0K MQTT Client with Session Locking & Anomaly Simulation")
...
if st.session_state.mqtt_payload:
    ...
    if session_id_resp == st.session_state.session_id:
        ...
        st.session_state.session_log.append({
            ...
            "Decryption Success": not decrypted.startswith("[❌]")
        })
...
if st.session_state.session_log:
    ...
    # --- Trend Analysis ---
    success_rate = (df["Decryption Success"].sum() / len(df)) * 100
    st.markdown(f"✅ **Decryption Success Rate:** {success_rate:.1f}%")

    noise_count = df["Noise Injected"].sum()
    st.markdown(f"⚠️ **Noise-Injection Count:** {noise_count}")
else:
    st.info("No session activity logged yet.")
